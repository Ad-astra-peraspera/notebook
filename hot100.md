...
class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        维护一个前缀积和后缀积
        '''
        n = len(nums)           # for reuse
        pre = [1] * n
        suf = [1] * n           # initialize: pre[0] == suf[n-1] == 1
...
原地哈希的思想，个人感觉很有意思的一道题，[灵茶山艾府](https://leetcode.cn/u/endlesscheng/)的题解很清晰
...
class Solution:
    ...
    def swap(self, i: int, lst: List[int]) -> None:
        j = lst[i] - 1
...
1. `nums[i] != i + 1`无法处理重复的数字出现的情况
2. `nums[i] >= 0` 边界判断不完整（而且错了，应该是大于0），还应该判断元素是否小于数组长度（否则在挪位置的时候会越界）
3. 应该先判断边界再判断位置是否正确，不然也会越界



`nums[i] != nums[nums[i] - 1]:` 非常的巧妙！ 膜拜

另外我在定义函数的时候犯了个错，忘记写self， 希望以后不要犯类似的问题了
...
1. 方向数组direction的方向
2. 自己想清楚了矩阵的长宽运算
3. 边界判断：矩阵内部可能有重复的数字，不能用ans来查重；边界条件是[0, m) [0, n)。]
...
class Solution:
    def spiralOrder(self, matrix: List[List[int]]) -> List[int]:
        ...
        '''
        # initialization
        i = j = d = 0                         # i: row index; j: col index; d: to check direction
        direction = [[0, 1], [1, 0],
                        [0, -1], [-1, 0 ]]     # direction: right, down, left, up
        ...
        for num in range(0, m * n):
            ...
            visited.add((i, j))
            new_i, new_j = i + direction[d][0], j + direction[d][1]
...
灵茶山艾府的解答很有启发性，很佩服。把旋转矩阵当作是一次转置加上一次行翻转就解决了。写代码的本质是用计算机去解决数学问题，首先你需要对要解决的数学问题有一个思路，要对其进行建模才能更好解决问题。
...
![image-20251204200546736](../../Library/Application Support/typora-user-images/image-20251204200546736.png)
...
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        # 1.calculate the m and n of matrix
        m = len(matrix)        # row
        n = len(matrix[0])     # col

        i, j = 0, n-1          # top right corner

        # 2.travrise the matrix
        while i < m and j >= 0:# [!]be care of the border!
...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        cur = head
        while cur:
            tmp = cur.next  # save the next point
            cur.next = pre  # reverse
            '''
            !caution: not 'pre = cur.next'
            '''
            pre = cur       # move pre pointer
            cur = tmp       # move cur pointer
        return pre
...
class Solution:
    def isPalindrome(self, head: Optional[ListNode]) -> bool:
        # 思路：从链表的开头与结尾开始同时遍历，如果有不一致则不是回文。
        # 要实现这个功能，要再写两个功能：寻找中点，在其基础上反转后半部分链表
        # 功能1：寻找中点. 使用方法：快慢指针
...
- 1.在写内部函数find_mid与reverse_List的时候，给其了一个参数self：这不对，因为只有其作为类方法的时候才需要使用self，在此题背景下，只是作为一个函数存在，不需要传入self。
- 2.注意边界条件的存在。在遍历时，要充分考虑链表的边界条件



## 4.[141. 环形链表](https://leetcode.cn/problems/linked-list-cycle/)

比较简单，只需要使用快慢指针的方法即可。注意判断时，只需要判断fast与其next，不需要判断两次next。

```python
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        # Floyd's cycle detection using slow and fast pointers
        # try to use slow and fast pointer
        # a slow pointer and a fast pointer. the slow one moves a node a time, and the fast one moves 2
        fast, slow = head, head
        while fast and fast.next:
            slow = slow.next
            fast = fast.next.next
...
以前做过，还记得一些步骤，但是后面为什么是再走多少步到入环口没搞懂。现在应该是真的懂了。

![image-20260121134041874](../../Library/Application Support/typora-user-images/image-20260121134041874.png)
